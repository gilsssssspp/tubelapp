...
$maxSize    = 2 * 1024 * 1024; // 1MB per file
...
foreach ($map as $inputName => $dbCol) {
    ...
    if ($f['size'] > $maxSize) {
        ...
        $errors[] = "$label terlalu besar ({$sizeMb} MB). Maksimal 2 MB.";
        continue;
    }
...
/* ==========================
   CEK APAKAH INI DARI TOMBOL SUBMIT
   ========================== */
$dariSubmit = isset($_POST['dari_submit']) && $_POST['dari_submit'] == '1';

// Debug: Log untuk cek apakah dari_submit terkirim
if (isset($_POST['dari_submit'])) {
    error_log("dari_submit value: " . $_POST['dari_submit']);
}
...
if (!empty($errors)) {
    ...
    // Redirect berdasarkan origin parameter
    if ($origin === 'detail') {
        header("Location: ../detail.php?iddaftar=" . urlencode($iddaftar) . "&flash_upload=1");
    } else {
...
// Jika ini dari tombol submit, redirect ke submit.php setelah upload berhasil
if ($dariSubmit) {
    error_log("Upload berhasil, redirect ke submit.php untuk ID: $iddaftar");
    
    // Redirect ke submit.php untuk proses verifikasi
    header("Location: ../submit.php?iddaftar=" . urlencode($iddaftar));
    exit;
} else {
    error_log("Bukan dari submit, hanya simpan biasa");
}

$_SESSION['upload_status']  = 'success';
$_SESSION['upload_message'] = 'Simpan Berhasil';
