...
$id = (int)($_GET['iddaftar'] ?? 0);
if ($id <= 0) die('ID tidak valid');

// Admin: cari by id saja
if ($type === 'admin') {
    $stmt = mysqli_prepare($db, "SELECT sk, tubel_ibel, nip FROM pendaftaran WHERE id=? LIMIT 1");
    mysqli_stmt_bind_param($stmt, "i", $id);
} else {
    // Pegawai: wajib cocok id + nip
    $stmt = mysqli_prepare($db, "SELECT sk, tubel_ibel, nip FROM pendaftaran WHERE id=? AND nip=? LIMIT 1");
    mysqli_stmt_bind_param($stmt, "is", $id, $nip);
}
...
$nipPemilik = trim($row['nip'] ?? '');
if ($nipPemilik === '') die('NIP pemilik tidak ditemukan');
...
foreach ($candidateBases as $base) {
    $paths[] = $base . "/sk/{$nipPemilik}/{$jenis}/{$skFile}";
    $paths[] = $base . "/sk/{$jenis}/{$nip}/{$skFile}"; 
    $paths[] = $base . "/sk/{$nip}/{$jenis}/{$skFile}"; 
    $paths[] = $base . "/sk/{$nip}/{$skFile}";          
}
...
if ($filePath === null) {
    ...
    echo "NIP    = " . htmlspecialchars($nipPemilik) . "<br>";
...
header('Content-Disposition: inline; filename="' . $skFile . '"');
